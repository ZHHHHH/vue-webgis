...
(function(factory) {
  ...
  var IconLayers = L.Control.extend({
    ...
    _getInactiveLayers: function() {
      var ar = [];
      var activeLayerId = this._getActiveLayer()
        ? this._getActiveLayer().id
        : null;
      var previousLayerId = this._getPreviousLayer()
    ...
    _getLayerCellByLayerId: function(id) {
      var els = this._container.getElementsByClassName(
    ...
    _createLayerElement: function(layerObj) {
      ...
      if (layerObj.title) {
        var titleContainerEl = L.DomUtil.create(
          "div",
          "leaflet-iconLayers-layerTitleContainer"
        );
        var titleEl = L.DomUtil.create("div", "leaflet-iconLayers-layerTitle");
        var checkIconEl = L.DomUtil.create(
          "div",
      ...
      if (layerObj.icon) {
        el.setAttribute(
          "style",
          "background-image: url('" + layerObj.icon + "')"
        );
      }
      return el;
    },
    ...
    _attachEvents: function() {
      ...
      var layersRowCollection = this._container.getElementsByClassName(
    ...
    initialize: function(layers, options) {
      ...
      this._expandDirection =
        this.options.position.indexOf("left") != -1 ? "right" : "left";
    ...
    collapse: function() {
      this._arrangeLayers().map(
        function(l) {
          var el = this._getLayerCellByLayerId(l.id);
          var activeLayerId =
            this._getActiveLayer() && this._getActiveLayer().id;
